...
for pap in paps:
    ...
    for p in pap:
        p = p.split(":")
        if p[0] == "byr":
            if len(p[1]) != 4 or int(p[1]) > 2002 or int(p[1]) < 1920:
                pendule = False

        elif p[0] == "iyr":
            if len(p[1]) != 4 or int(p[1]) > 2020 or int(p[1]) < 2010:
                pendule = False

        elif p[0] == "eyr":
            if len(p[1]) != 4 or int(p[1]) > 2030 or int(p[1]) < 2020:
                pendule = False

        elif p[0] == "hgt":
            if p[1][-2:] == "cm":
                if len(p[1]) != 5 or int(p[1][:-2]) < 150 or int(p[1][:-2]) > 193:
                    pendule = False

            elif p[1][-2:] == "in":
                if len(p[1]) != 4 or int(p[1][:-2]) < 59 or int(p[1][:-2]) > 76:
                    pendule = False
            else:
                pendule = False

        elif p[0] == "hcl":
            if not re.search("^#[a-f0-9]{6}$", p[1]):
                pendule = False
            ...
            if not re.search("^[0-9]{9}$", p[1]):
                pendule = False
        elif p[0] == "cid":
            pass
        else:
            warnings.warn("unrecognized truc %s " % p[0])
    if pendule:
        print(pap, end="\n\n")
        count += 1
'''
