rules = """shiny plum bags contain no other bags.
...
"""star 1
def parse_rule(rule):
    contenant, contenu = rule.split("contain")
    contenant = ' '.join(contenant.strip().split()[0:2])
    for cont in contenu.strip().split(','):
        cont = ' '.join(cont.strip().split()[1:3])
        if cont != "other bags.":
            yield cont, contenant

for rule in rules:
    for edge in parse_rule(rule):
        #print("adding " , edge)
        G.add_edge(*edge)
...
while stack:
    couleur = stack.pop()
    if couleur not in vues:
        vues.add(couleur)
        stack.extend(G.neighbors(couleur))
print(vues)
print(len(vues)-1)
...
def parse_rule(rule):
    contenant, contenu = rule.split("contain")
    contenant = ' '.join(contenant.strip().split()[0:2])
    if contenu != " no other bags.":
        for cont in contenu.strip().split(','):
            weight = int(cont.strip().split()[0])
            cont = ' '.join(cont.strip().split()[1:3])

            yield cont, contenant, weight

for rule in rules:
    for cont, contenant, weight in parse_rule(rule):
        #print("adding " , edge)
        G.add_edge(contenant, cont, weight=weight)

#nx.draw_networkx(G)
#plt.show()
def f(G, node):
    if "weigth" not in G.nodes[node]:
        G.nodes[node]["weight"] = sum(
            G[node][n]["weight"] * (1 + f(G, n))
            for n in G.neighbors(node)
        )
    return G.nodes[node]["weight"]

print(f(G, "shiny gold"))