...
for truc in l:
    truc = truc.split(':')
    req.append(truc[0])
    passwd.append(truc[1].strip())
u = 0

while u < len(passwd):
    curreq = req[u]
    curreq= str(curreq.split(" ")[0])

    curreq2 = req[u]
    letter = str(curreq2.split(" ")[1])

    curreq = list(map(int,curreq.split("-")))
    print("testing : " , curreq, letter , passwd[u])




    """Star1 : 519
    if (passwd[u].count(letter) >= curreq[0] and passwd[u].count(letter) <= curreq[1]):
    """
    """Star2 : 708
    if((passwd[u][curreq[0]-1] == letter) != (passwd[u][curreq[1]-1] == letter)):
    """
        count += 1
        print(curreq, letter , passwd[u], " is correct")
    u += 1
